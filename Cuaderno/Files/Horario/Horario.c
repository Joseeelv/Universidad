...
Horario* getHorarios(int *arraySize){
	...
	if(size==0) lineas =0; //comprobamos que el tamaÃ±o del fichero es 0 para ver si este estÃ¡ vacÃ­o
}
	fclose(fich); //cerramos el fichero.

	*arraySize = lineas; // el tamaÃƒÂ±o del array es el numero de lineas.
...
//precondicion recibe un array de tipo horario ,inicializado, y el tamaÃño del array
...
//precondicion recibe un array de tipo Horario inicializado y el tamaÃƒÂ±o del array
...
//precondicion recibe un array de tipo horario, inicializado, su tamaño y el id_prof para añadirle un horario al profesor
//postcondicion se le añade un horario nuevo a un profesor
void crearhorario(Horario** Horarios, int* nHorarios,int id_prof){
	int i;
	printf("AÑADIR NUEVO HORARIO\n", 165); 
	
	if(((*Horarios)= realloc ((*Horarios), (*nHorarios+1)*sizeof(Horario)))== NULL){ //comprobamos si se puede aumentar el tamaño
		printf("ERROR, NO SE HA PODIDO RESERVAR MEMOMIA\n");					  //del array para añadir horarios
	}																			  
 
	i = (*nHorarios); //asignamos a la variable el tamaño del array
	*nHorarios = *nHorarios + 1; //aumentamos el tamaño del array en 1
