...
//Cabecera: int login(int* nUsuarios, Usuario** usuarios, int* id);
//Precondicion:
//Postcondicion: Nos devuelve al menu correspondiente segun el perfil del usuario
int login(int* nUsuario, Usuario** usuarios, int* id){
	int i;
	char user[6];
	char password[9];
	int respuesta;
	...
	for(i=0; i<=*nUsuario; i++){
		if(strcmp((*usuarios)[i].usuario,user) == 0){
			...
			scanf("%s",&password);
			fflush(stdin);
			if(strcmp((*usuarios)[i].pass, password) == 0){
				
				*id=(*usuarios)[i].id;
				
				if((*usuarios)[i].perfil[0] == 'a')
	  			      return 1;
			    if((*usuarios)[i].perfil[0] == 'p')
			          return 2;
					  		  
				i=*nUsuario+1; 						   
		   	}else
		   		printf("Contrase%ca incorrecta, reinicie el programa\n", 164);
		   		return -1;
	    }
	}
	
	if(i>*nUsuario){
		printf("No se ha encotrado un Usuario...");
		printf("\n Si quieres crear un Usuario pulsa 1 si no pulsa 0: ");
		scanf("%d",&respuesta);
		fflush(stdin);
			
		if(respuesta == 1){   
			addUsuario(&*usuarios, &*nUsuario);
				*id=(*usuarios)[i-1].id;
				return 1;
		}else{
			   return -1;
