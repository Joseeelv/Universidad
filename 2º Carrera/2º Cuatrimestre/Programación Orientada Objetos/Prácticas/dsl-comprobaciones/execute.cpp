...
void execute::run(const MatchFinder::MatchResult &Result)
{
  ...
  classWithMutableVariable(Result);
...
void execute::hasMutableVariable()
{
  if (!foundAll(mutableVariables)) {
    getMessagesOfNotFound(mutableVariables);
    correct = false;
  }
}
...
void execute::setMutableVariables(string className, string variableName,
				 string message)
{
  Info info(variableName, message, false);
  info.setClass(className);
  mutableVariables.insert(std::make_pair(variableName, info));
}
						  ...
						  &Result)
{
  ...
  if (existClass(foundClassWithListInitializerConstructor)
	...
	//Removed condition: params and initializers are not necessarily the same		   
...
void execute::classWithMutableVariable(const MatchFinder::MatchResult &Result)
{
  const CXXRecordDecl *foundClass =
    Result.Nodes.getNodeAs<clang::CXXRecordDecl>("foundClass");
  const FieldDecl *foundMutableVariable =
    Result.Nodes.getNodeAs<clang::FieldDecl>("foundMutableVariable");
  if (existClass(foundClass) && existMember(foundMutableVariable)) {   
    //llvm::outs() << foundMutableVariable->getNameAsString() << "\n";
    for (AI it=mutableVariables.begin(); it!=mutableVariables.end(); ++it)
      if (std::regex_match(foundMutableVariable->getNameAsString(),
			   std::regex(it->first)) &&
	  std::regex_match(foundClass->getNameAsString(),
			   std::regex((it->second).getClass()))) {
	if (foundMutableVariable->isMutable())
	  (it->second).setFound(true);
      }
  }
}
