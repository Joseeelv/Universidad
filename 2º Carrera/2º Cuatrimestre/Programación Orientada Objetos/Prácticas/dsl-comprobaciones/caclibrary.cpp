...
void checkCode::mutableVariable(string className, vector<string> variableNames,
			        string message)
{
  for (unsigned int i = 0 ; i < variableNames.size(); i++) {
    e.setMutableVariables(className, variableNames[i], message);
    Finder.addMatcher(mutableVariable_Matcher(className, variableNames[i]), &e);
  }
}
