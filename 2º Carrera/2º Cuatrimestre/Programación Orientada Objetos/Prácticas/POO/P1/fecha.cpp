...
//Implementacion de los operadores de extraccion e inserccion
std::ostream& operator <<(std::ostream& os, const Fecha& f) noexcept{
    //Imprimimos la fecha por pantalla
    os << f.cadena(); //creado para evitar ambiguedades
    return os;
}
std::istream& operator >>(std::istream& is, Fecha& f){
    //Introducimos la fecha por teclado
    char fechita[12]="";
    is.getline(fechita,12); //guardamos la linea que introducimos por teclado
    //Comprobamos que el formato de la fecha es correcto
    try{
        f=Fecha(fechita);
    } catch(Fecha::Invalida fechainvalida){
        is.setstate(std::ios::failbit); //captura la excepcion

        throw fechainvalida; //lanza la excepción
    }
    return is; //devuelve lo introducido
}
