...
{
    nodo hizqdo;
    assert(numNodos > 0); // Árbol no vacío.assert(n >= 0 && n <= maxNodos-1); // n es una celda del vectorassert(n == 0 || nodos[n].padre != NODO_NULO); // que está ocupada.assert(numNodos < maxNodos); // Árbol no lleno.
    // Añadir el nuevo nodo en la primera celda libre.
    for (hizqdo = 1; nodos[hizqdo].padre != NODO_NULO; hizqdo++)
        ;
    nodos[hizqdo].elto = e;
    nodos[hizqdo].padre = n;
    // Insertar el nuevo nodo al inicio de la lista de hijos de n.Lista<nodo>& Lh = nodos[n].hijos; // Lista de hijos.
